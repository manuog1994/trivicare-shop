...
var render=function render(){var _vm=this,_c=_vm._self._c;return _c("client-only",[_c("div",{staticClass:"product-details-page-wrapper"},[_c("HeaderWithTopbar",{attrs:{containerClass:"container-fluid"}}),_vm._v(" "),_c("TheHeader",{attrs:{searchFather:_vm.searchChildren},on:{opacity:_vm.searchOpacity}}),_vm._v(" "),_c("div",{attrs:{id:"post-nav"},on:{click:_vm.closeMenus}},[_c("NavBottom"),_vm._v(" "),_vm.productDetails?_c("ProductDetailsWrapper",{attrs:{product:_vm.productDetails}}):_vm._e(),_vm._v(" "),_vm.productDetails?_c("ProductDetailsDescriptionReview",{attrs:{product:_vm.productDetails,reviews:_vm.productDetails.reviews}}):_vm._e(),_vm._v(" "),_c("TheFooter")],1)],1)]);};var staticRenderFns=[];render._withStripped=true;
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = ({auth:false,pageTransition:'slide-fade',async asyncData({store,params,req}){try{if(!req){const visitorIP='No IP';const productDispatch=await store.dispatch('getProducts',{page:'',category:'',search:'',slug:params.slug,sort:'',tag:'',status:2});const productDetails=store.getters.getProducts;return{productDetails:productDetails.data[0],visitorIP};}else{const visitorIP=req.headers['x-forwarded-for']||req.connection.remoteAddress||req.socket.remoteAddress||req.connection.socket.remoteAddress;const productDispatch=await store.dispatch('getProducts',{page:'',category:'',search:'',slug:params.slug,sort:'',tag:'',status:2});const productDetails=store.getters.getProducts;return{productDetails:productDetails.data[0],visitorIP};}}catch(error){console.log(error);}},head(){return{title:this.productDetails.name+' | TriviCare Natural Cosmetics',link:[{rel:'cannonical',href:'https://trivicare.com/product/'}],meta:[{charset:'utf-8'},{name:'viewport',content:'width=device-width, initial-scale=1'},{hid:'description',name:'description',content:this.productDetails.meta_description}]};},data(){return{searchChildren:''};},mounted(){this.$nextTick(()=>{this.$nuxt.$loading.start();setTimeout(()=>{this.$nuxt.$loading.finish();},2000);});this.$axios.post('/api/visit',{ip_address:this.visitorIP,page_visited:this.productDetails.name});},methods:{closeMenus(){this.searchOpacity(false);this.$root.$emit('closeMenu',this.closeMenu);},searchOpacity(searchFather){if(searchFather==true){document.getElementById("post-nav").classList.add("search-screen");}else{document.getElementById("post-nav").classList.remove("search-screen");}}}});
