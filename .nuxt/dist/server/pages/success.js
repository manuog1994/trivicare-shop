...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* WEBPACK VAR INJECTION */(function(URLSearchParams) {/* harmony default export */ __webpack_exports__["default"] = ({middleware:'token',auth:false,components:{Loading:()=>Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ~/components/Loading.vue */ "./components/Loading.vue"))},beforeCreate(){this.loading=true;},data(){return{paymentIntent:null,counter:false,countdownNumbers:'',loading:true,error:false};},beforeMount(){const url=new URLSearchParams(window.location.search).get('payment_intent_client_secret');if(url!=null){this.paymentIntent=url;}},mounted(){if(this.paymentIntent!=null){this.orderPaid();}},watch:{},methods:{async orderPaid(){const token_id=this.paymentIntent;await this.$axios.post('/api/order-paid/'+token_id).then(res=>{console.log(res.data);this.loading=false;this.countdown(10);this.$store.commit('CLEAR_GUEST');this.$store.commit('CLEAR_CART');this.$store.commit('CLEAR_CUPON');}).catch(err=>{console.log(err.response.data);this.loading=false;this.countdown(20);this.paymentIntent=null;this.error='Ha ocurrido un error, por favor intente nuevamente más tarde.';this.$axios.post('/api/error-message',{message:error.response.data.message});});},// countdown of 10 seconds to redirect to my-orders
countdown(counter){const interval=setInterval(()=>{this.counter=true;this.countdownNumbers=counter;counter--;if(counter===0){clearInterval(interval);this.$router.push('/');}},1000);},head(){return{titleTemplate:'Estamos procesando su pago... | TriviCare Natural Cosmetics',link:[{rel:'cannonical',href:'https://trivicare.com/success'}]};}}});
