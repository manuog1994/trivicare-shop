exports.ids = [55,7,15,19,25,26,27,40];
exports.modules = {
...
/* harmony default export */ var my_accountvue_type_script_lang_js_ = ({middleware:'auth',transition:{name:'fade',mode:'out-in'},data(){return{email:'',old_password:'',password:'',password_confirmation:'',errors:[],name:'',lastname:'',address:'',optional_address:'',city:'',state:'',zipcode:'',phone:'',country:''};},components:{HeaderWithTopbar:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 85)),Breadcrumb:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 83)),TheFooter:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 82))},mounted(){},methods:{async updateEmail(){external_sweetalert2_default.a.fire({title:'¿Estas seguro de que quieres cambiar tu email?',showCancelButton:true,confirmButtonText:'Si, cambiar'}).then(result=>{/* Read more about isConfirmed, isDenied below */if(result.isConfirmed){this.$axios.post('/api/update-email/'+this.$auth.user.id,{email:this.email}).then(()=>{external_sweetalert2_default.a.fire('¡Email actualizado!','','success');this.$auth.fetchUser();this.errors=[];this.email='';}).catch(error=>{this.errors=Object.values(error.response.data).flat();});}});},async updatePassword(){await this.$axios.post('/api/update-password/'+this.$auth.user.id,{old_password:this.old_password,password:this.password,password_confirmation:this.password_confirmation}).then(()=>{external_sweetalert2_default.a.fire('¡Contraseña actualizada!','','success');this.old_password='';this.password='';this.password_confirmation='';this.errors=[];this.$auth.fetchUser();}).catch(error=>{this.errors=Object.values(error.response.data).flat();});},async deleteProfile(id){external_sweetalert2_default.a.fire({title:'¿Estas seguro de que quieres eliminar esta dirección?',showCancelButton:true,confirmButtonText:'Si, eliminar'}).then(result=>{/* Read more about isConfirmed, isDenied below */if(result.isConfirmed){this.$axios.delete('/api/delete-profile/'+id);this.$auth.fetchUser();external_sweetalert2_default.a.fire('Su dirección ha sido eliminada!','','success');}});},async deleteAccount(){external_sweetalert2_default.a.fire({title:'¿Estas seguro de que quieres eliminar tu cuenta?',showCancelButton:true,confirmButtonText:'Si, eliminar'}).then(result=>{/* Read more about isConfirmed, isDenied below */if(result.isConfirmed){this.$axios.delete('/api/destroy/'+this.$auth.user.id);external_sweetalert2_default.a.fire('Su cuenta ha sido eliminada!','','success');this.$auth.logout();this.$router.push('/');}});},async createProfile(){const response=await this.$axios.post('/api/register-profile',{user_id:this.$auth.user.id,name:this.name,lastname:this.lastname,address:this.address,optional_address:this.optional_address,city:this.city,state:this.state,zipcode:this.zipcode,phone:this.phone,country:this.country});this.name='';this.lastname='';this.address='';this.optional_address='';this.city='';this.state='';this.zipcode='';this.phone='';this.country='';this.$auth.fetchUser();this.$notify({type:'success',text:'Dirección creada correctamente'});},async resendEmail(){await this.$axios.post('/api/resend-email/'+this.$auth.user.id).then(res=>{console.log(res);this.$notify({title:'Email reenviado'});}).catch(error=>{this.errors=Object.values(error.response.data).flat();});}},head(){return{title:"Mi Cuenta"};},mounted(){var tituloOriginal=document.title;// Lo guardamos para restablecerlo
