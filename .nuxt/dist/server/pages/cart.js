...
};},methods:{incrementProduct(product){const prod={...product,cartQuantity:1};if(product.cartQuantity<product.stock){this.$store.dispatch('addToCartItem',prod);}else{this.$notify({title:'No hay más stock disponible'});}},decrementProduct(product){const prod={...product,cartQuantity:1};if(product.cartQuantity>1){this.$store.dispatch('decreaseProduct',prod);}},removeProduct(product){// for notification
this.$notify({title:'Producto eliminado del carrito!'});this.$store.dispatch('removeProductFromCart',product);},discountedPrice(product){return product.price-product.price*product.discount/100;},clearCart(){if(confirm("¿Estás seguro de que quieres vaciar el carrito?")){// for notification
this.$notify({title:'Carrito vaciado!'});this.$store.commit('CLEAR_CART');}},async getCupons(){await this.$axios.get('/api/cupons').then(response=>{this.cupons=Object.values(response.data.data).flat();}).catch(error=>{console.log(error);});},validationCupon(){this.cupons.filter(cupon=>{if(cupon.code==this.inputCupons){this.cupon=cupon;}});const expires=new Date(this.cupon.validity).toLocaleDateString().replace(/\//g,'-');const today=new Date().toLocaleDateString().replace(/\//g,'-');if(this.cupon){if(expires<today){this.error='El cupón ha caducado';this.inputCupons='';}else if(this.cupon.status==1){this.error='El cupón ya no está disponible';this.inputCupons='';}else{this.error='';this.discountCupon=this.cupon.discount;this.$store.commit('SET_CUPON',this.cupon);this.inputCupons='';this.$notify({title:'Cupón aplicado!'});}}else{this.error='El cupón no es válido';this.inputCupons='';}},deleteCupon(){this.cupon='';this.$store.commit('SET_CUPON',this.cupon);this.$notify({title:'Cupón eliminado!'});}},head(){return{title:"Carrito"};}});
