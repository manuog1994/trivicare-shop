exports.ids = [56,7,15,19,25,26,27,40];
exports.modules = {
...
/* harmony default export */ var _idvue_type_script_lang_js_ = ({auth:true,transition:{name:'fade',mode:'out-in'},components:{HeaderWithTopbar:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 85)),Breadcrumb:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 83)),TheFooter:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 82))},data(){return{orders:'',pagination:{},user:{}};},head(){return{title:'Mis Pedidos'};},mounted(){this.getOrder();},computed:{page(){var _this$$route$query$pa;let page=(_this$$route$query$pa=this.$route.query.page)!==null&&_this$$route$query$pa!==void 0?_this$$route$query$pa:1;if(page>this.pagination.last_page){this.$router.replace({query:{page:this.pagination.last_page}});this.$router.push();return this.pagination.last_page;}return page;}},watch:{page(){this.$nextTick(()=>{this.$nuxt.$loading.start();setTimeout(()=>{this.$nuxt.$loading.finish();},500);});setTimeout(()=>{this.getOrder();},500);}},methods:{async getOrder(){await this.$axios.get('/api/orders?perPage=10&page='+this.page+'&sort=-id&filter[user_id]='+this.$auth.user.id).then(response=>{this.orders=response.data.data;const paginations=response.data;this.pagination={links:paginations['meta'].links,current_page:paginations['meta'].current_page,last_page:paginations['meta'].last_page};}).catch(error=>{console.log(error);});},changePage(url){this.$router.replace({query:{page:url.split('page=')[1]}});},getState(order){if(order.status==1){return'Recibido';}else if(order.status==2){return'Preparando';}else if(order.status==3){return'Enviado';}else if(order.status==4){return'Entregado';}else if(order.status==5){return'Cancelado';}},getName(order){let profiles=order.user.user_profile;return profiles.filter(profile=>{return profile;}).map(profile=>{this.user=profile;return profile.name+' '+profile.lastname;}).toString();},async getUrl(order){let FileDownload=__webpack_require__(70);await this.$axios.get('/api/invoices/'+order.invoice.id,{responseType:'blob'}).then(response=>{FileDownload(response.data,order.invoice.filename);});}}});
// CONCATENATED MODULE: ./pages/my-orders/_id.vue?vue&type=script&lang=js&
 /* harmony default export */ var my_orders_idvue_type_script_lang_js_ = (_idvue_type_script_lang_js_); 
// EXTERNAL MODULE: ./node_modules/vue-loader/lib/runtime/componentNormalizer.js
var componentNormalizer = __webpack_require__(4);

// CONCATENATED MODULE: ./pages/my-orders/_id.vue
...
var component = Object(componentNormalizer["a" /* default */])(
  my_orders_idvue_type_script_lang_js_,
  render,
  staticRenderFns,
  false,
  null,
