...
import { useNuxt, addPluginTemplate, tryImportModule, resolvePath, addTemplate, resolveAlias, addWebpackPlugin, addVitePlugin, addPlugin, extendWebpackConfig, resolveFiles, defineNuxtModule, logger, tryResolveModule, installModule, checkNuxtCompatibility } from '@nuxt/kit';
...
async function setupNitroBridge() {
  ...
  nuxt.hook("build:compiled", async ({ name }) => {
    ...
    if (name === "server") {
      ...
      await promises.writeFile(resolve(nuxt.options.buildDir, "dist/server/client.manifest.mjs"), "export default " + JSON.stringify(normalizeWebpackManifest(JSON.parse(manifest))), "utf8");
    }
  });
...
export { distDir as d, generateDevSSRManifest as g, hashId as h, isCSS as i, module as m, prepareManifests as p, uniq as u };
