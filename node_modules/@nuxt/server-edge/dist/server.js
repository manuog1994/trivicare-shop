/*!
 * @nuxt/server-edge v2.16.0-27720022.54e852f (c) 2016-2022
...
const vueRendererEdge = require('@nuxt/vue-renderer-edge');
...
async function renderAndGetWindow(url = "http://localhost:3000", jsdomOpts = {}, {
  ...
  const jsdom = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('jsdom')); }).then((m) => m.default || m).catch((e) => {
...
const nuxtMiddleware = ({ options, nuxt, renderRoute, resources }) => async function nuxtMiddleware(req, res, next) {
  ...
  try {
    ...
    if (redirected && context.target !== utilsEdge.TARGETS.static) {
    ...
    if (!this.socket) {
      switch (address.address) {
        case "127.0.0.1":
          ...
          break;
        case "0.0.0.0":
          this.host = ip__default["default"].address();
          break;
      }
...
class Server {
  constructor(nuxt) {
    ...
    this.publicPath = utilsEdge.isUrl(this.options.build.publicPath) ? this.options.build._publicPath : this.options.build.publicPath.replace(/^\.+\//, "/");
    this.resources = {};
    this.listeners = [];
  ...
  async ready() {
    ...
    this.renderer = new vueRendererEdge.VueRenderer(this.serverContext);
    ...
    if (this.options.dev && routerBase !== "/") {
      this.useMiddleware({
        ...
        handler: (req, res, next) => {
          ...
          const to = utilsEdge.urlJoin(routerBase, req.url);
